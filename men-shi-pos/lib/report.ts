...
export function buildMonthDailyCash(
  state: AppState,
  day: string,
): MonthDayCashRow[] {
  const selected = parseInputDate(day);
  ...
  for (let date = 1; date <= lastDay; date += 1) {
    ...
    const staffAmount = sales
      .filter((line) => isStaffSaleLine(line))
      .reduce((sum, line) => sum + line.amount, 0);
    const refundAmount = returnDetailTotals(refunds).amount;
    const expenseAmount = expenseTotals(expenses);
    const netAmount = saleTotals.amount - refundAmount;
    rows.push({
      date: key,
      day: date,
      ...
      netAmount,
      expenseAmount,
      balance: netAmount - expenseAmount,
    });
  }
  return rows;
}

export function monthDailyCashTotals(rows: MonthDayCashRow[]) {
  return rows.reduce(
    (sum, row) => ({
      saleQty: sum.saleQty + row.saleQty,
      ...
      expenseAmount: sum.expenseAmount + row.expenseAmount,

...
export function ProductsView() {
  ...
  const comboPickables = useMemo(
    () =>
      state.products.filter(
        (item) =>
          !isCombo(item) &&
          item.id !== editingId &&
          !form.comboParts.some((part) => part.productId === item.id),
      ),
    [state.products, editingId, form.comboParts],
  );

  const comboPickMatches = useMemo(
    () =>
      matchProducts(comboPickables, form.pickQuery, {
        emptyAll: true,
      }),
    [comboPickables, form.pickQuery],
  );
      ...
      <div className="border-b bg-card px-3 py-2 md:px-4">
        ...
        >
          ...
          <div className="mb-2 rounded-md bg-muted/50 px-2.5 py-2 text-xs leading-5 text-muted-foreground">
            ...
            <p>
              5. 名稱 烤肉組、售價 599、勾「這是套組」、搜尋並勾選上面四項、按「加入已勾選」、再按加入
            </p>
              ...
              <div className="space-y-2 rounded-md bg-muted/40 px-2 py-2">
                ...
                <div className="space-y-2">
                  <label className="block space-y-0.5">
                    ...
                    <input
                      className={cn(addInputClass, "w-full")}
                      value={form.pickQuery}
                      onChange={(event) =>
                        setForm((current) => ({
                          ...current,
                          pickQuery: event.target.value,
                        }))
                      }
                      placeholder="例如 白蝦、鳳梨酥、P0051"
                      autoComplete="off"
                    />
                  </label>
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <span className="text-muted-foreground">
                      {form.pickQuery.trim()
                        ? `找到 ${comboPickMatches.length} 項`
                        : `共 ${comboPickables.length} 項可勾`}
                      {form.pickIds.length > 0
                        ? ` · 已勾 ${form.pickIds.length} 項`
                        : ""}
                    </span>
                    <button
                      type="button"
                      ...
                      onClick={() =>
                        setForm((current) => ({
                          ...current,
                          pickIds: comboPickMatches.map((item) => item.id),
                        }))
                      }
                    >
                      全選目前結果
                  ...
                  <div className="max-h-48 overflow-auto rounded-md border bg-background">
                    {comboPickMatches.length === 0 ? (
                      <p className="px-2 py-3 text-sm text-muted-foreground">
                        {comboPickables.length === 0
                          ? "沒有可加的商品"
                          : "找不到商品，換個字再搜"}
                      </p>
                    ) : (
                      <ul className="divide-y">
                        {comboPickMatches.map((item) => {
                          const checked = form.pickIds.includes(item.id);
                          return (
                            <li key={item.id}>
                                ...
                                <input
                                  ...
                                  onChange={() =>
                                    setForm((current) => ({
                                      ...current,
                                      pickIds: checked
                                        ? current.pickIds.filter(
                                            (id) => id !== item.id,
                                          )
                                        : [...current.pickIds, item.id],
                                    }))
                                  }
                                />
                  ...
                  <div className="flex flex-wrap items-end gap-2">
                    ...
                    <button
                      ...
                      className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground"
                      onClick={() => {
                        if (form.pickIds.length === 0) {
                          toast.error("請先勾選要加進套組的商品");
                          return;
                        }
                        setForm((current) => ({
                          ...current,
                          comboParts: [
                            ...
                            ...current.pickIds
                              .filter(
                                (id) =>
                                  !current.comboParts.some(
                                    (part) => part.productId === id,
                                  ),
                              )
                              .map((id) => ({
                                productId: id,
                                qty: current.pickQty || "1",
                              })),
                          ],
                          pickIds: [],
                          pickQty: "1",
